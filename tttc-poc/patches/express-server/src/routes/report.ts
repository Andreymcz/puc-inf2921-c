// PoC patch: getReportDataHandler bypasses GCS and serves local files directly.
// NAO usar em producao. Substitui src/routes/report.ts via COPY no Dockerfile.express-poc.
...
export async function getReportDataHandler(req: Request, res: Response) {
  ...
  switch (parsed.tag) {
    case "success": {
      ...
      // PoC: local storage mode -- return absolute URL to /local-report/:file route.
      if (process.env.USE_LOCAL_STORAGE === "true") {
        ...
        const localPath = urlResult.value; // /local-report/{encodedFileName}
        const host = req.get("host") || "express-server:8080";
        res.json({ url: `http://${host}${localPath}` });
        return;
      }
