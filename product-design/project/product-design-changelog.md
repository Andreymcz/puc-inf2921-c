...
2026-06-16 | plan-000062 | §0e | Fala Gávea Segurança: filtro temporal `until` adicionado a ReportFilter; query SQLAlchemy aplica `created_at <= until`; parâmetro `?until=` exposto em GET /security_reports/geojson e GET /security_reports/; testes unitários e de integração adicionados
2026-06-16 | plan-000061 | §0e | Fala Gávea Segurança: AI auto-categorização — POST /security_reports/{id}/auto_categorize chama Ollama com CATEGORIZE_PROMPT e salva ai_suggested_category; PATCH /security_reports/{id}/category permite delegado confirmar/corrigir categoria zerando sugestão; campo ai_suggested_category em entidade, modelo DB, repositório e schema de response; 12 novos testes (7 unitários, 5 integração)
